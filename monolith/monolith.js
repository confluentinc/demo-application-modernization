...
app.post("/balance", (req, res) => {
  ...
  pool.connect((err, client, done) => {
    ...
    if (err) {
      ...
      res
        .status(500)
        .send({ message: "Error connecting to database connection pool." });
      return;
    }
    const abort = (err) => {
      if (err) {
        log(
          "error",
          "Encountered error during transaction. Performing rollback.",
          err.stack
        );
        res.status(500).send({
          message: "Encountered error during transaction. Performing rollback.",
        });
